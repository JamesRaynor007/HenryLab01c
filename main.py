...
def cantidad_filmaciones_mes(mes: str):
    ...
    return HTMLResponse(content=f"<p>La cantidad de películas que fueron estrenadas en el mes de <strong>{mes_en_ingles}</strong> es: <strong>{cantidad}</strong>.</p>")
...
def cantidad_filmaciones_dia(dia: str):
    ...
    return HTMLResponse(content=f"<p>La cantidad de películas que fueron estrenadas en el día <strong>{dia_en_ingles}</strong> es: <strong>{cantidad}</strong>.</p>")
...
async def score_titulo(title: str):
    ...
    return HTMLResponse(content=f"<p>La película '<strong>{movie_data['title']}</strong>' fue estrenada en el año <strong>{int(movie_data['release_year'])}</strong>, con una popularidad de <strong>{float(movie_data['popularity']):.2f}</strong>.</p>")
...
def get_titles():
    ...
    titulos_html += "</ul>"
    return HTMLResponse(content=f"<p>Titulos disponibles:</p>{titulos_html}")
...
async def votos_titulo(title: str):
    ...
    if movie_data['vote_count'] < 2000:
        ...
        return HTMLResponse(content=f"<p>La película '<strong>{movie_data['title']}</strong>' tuvo <strong>{int(movie_data['vote_count'])}</strong> votos y su puntaje promedio fue <strong>{float(movie_data['vote_average']):.2f}.</strong></p>")
...
def get_actor(actor_name: str):
    ...
    return HTMLResponse(content=f"<p>{resultado_texto}</p>")
...
def listar_actores():
    ...
    actores_html += "</ul>"
    return HTMLResponse(content=f"<p>Actores disponibles:</p>{actores_html}")
...
def obtener_directores():
    ...
    directores_html += "</ul>"
    return HTMLResponse(content=f"<p>Directores disponibles:</p>{directores_html}")
...
def recomendacion(title: str):
    ...
    return HTMLResponse(content=f"<p>Recomendaciones basadas en '<strong>{title}</strong>': {recommendations_html}</p>")
